...
**Version:** 1.3 · **Date:** 2026-07-12 · **Status:** Approved — Frozen MVP
...
**AC:** no `/100` score, no dimension score, no person rating anywhere in the experience · no evaluative labels (weak, elite, strong, building, poor, needs improvement, or equivalents) · no XP, streaks, badges, levels, milestone animations, or rewards attached to the check-in · does not gate or unlock chat, Mental Rep, Ritual, debrief, or any other feature · is never used as a pilot-success metric · remains visually and functionally separate from the diagnose → prescribe → follow-up coaching loop (§2) — it is background context, not a loop stage · the old numeric mental-fitness scoring experience (percentage score, dimension scores, rating bands) is not visible anywhere in the pilot.

**Recent-entry context in chat.** Arjun may receive only the athlete's latest few Mind Journal entries as optional background context — informing tone and warmth, never a substitute for the focused questions and barrier confirmation in §4.3, and never sufficient on its own to trigger a hypothesis or produce a prescription. The athlete is clearly informed, at the check-in surface, that recent entries may be used as context in conversations with Arjun. No long-term scoring, profiling, personality inference, or hidden ranking may be derived from journal entries. The exact number of entries treated as "latest few," and the retrieval mechanism, are engineering-plan decisions — this specification requires only that the limit be small and fixed.
...
**5.3 Overdiagnosis guards.** Hypothesis language only ("sounds like", "I think what might be happening") · one barrier at a time · athlete confirmation required before prescription · Arjun may say "I'm not sure yet — tell me about the next match and we'll look again" · barriers are behaviour patterns, never identity statements about the athlete · **[RESOLVED 2026-07-12 — v1.3]** recent Mind Journal entries (§4.12) may inform tone as background context only — they never determine a barrier hypothesis and never trigger a prescription on their own; the barrier must always come from this conversation's focused questions and the athlete's explicit confirmation.
...
| Old numeric mental-fitness score | **[RESOLVED 2026-07-12 — v1.3]** The `/100` score, dimension scores, rating bands, and evaluative labels are removed from the pilot experience; the Mind Journal itself remains as a separate, score-free private check-in (§4.12) outside the required coaching loop |
...
**End of specification — v1.3 (Approved — Frozen MVP).**
