...
**Recent-entry context in chat.** **[RESOLVED 2026-07-12 — v1.4]** Arjun may receive only the athlete's **latest five** Mind Journal entries as optional background context — informing tone and warmth, never a substitute for the focused questions and barrier confirmation in §4.3, and never sufficient on its own to trigger a hypothesis or produce a prescription. The athlete is clearly informed, at the check-in surface, that the latest five entries may be used as context in conversations with Arjun. No long-term scoring, profiling, personality inference, or hidden ranking may be derived from journal entries. The retrieval mechanism is an engineering-plan decision — this specification fixes only the limit, at five.
...
**5.3 Overdiagnosis guards.** Hypothesis language only ("sounds like", "I think what might be happening") · one barrier at a time · athlete confirmation required before prescription · Arjun may say "I'm not sure yet — tell me about the next match and we'll look again" · barriers are behaviour patterns, never identity statements about the athlete · **[RESOLVED 2026-07-12 — v1.3]** the latest five Mind Journal entries (§4.12) may inform tone as background context only — they never determine a barrier hypothesis and never trigger a prescription on their own; the barrier must always come from this conversation's focused questions and the athlete's explicit confirmation.
