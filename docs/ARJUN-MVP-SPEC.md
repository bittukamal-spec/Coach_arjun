...
**Version:** 1.4 · **Date:** 2026-07-12 · **Status:** Approved — Frozen MVP
...
Decisions resolved 2026-07-12 (v1.3), completing the implementation-planning decision set: the bottom-navigation slot vacated by the hidden Progress tab is replaced with Mental Playbook, which remains the pilot's only progress-review surface · the Starter Plan card and its pre-authored multi-session sequence are hidden from the pilot Dashboard, kept dormant (not deleted or expanded), and must not compete with the core diagnose → prescribe → follow-up loop · the Mind Journal remains in the pilot as a separate, score-free private check-in — no `/100` score, dimension score, person rating, or evaluative label — optional, non-gating, never a pilot-success metric, and clearly separate from the prescription loop; Arjun may receive only the latest few entries as limited, disclosed background context that informs tone but never determines a barrier or produces a prescription on its own · the next-open follow-up opener is a deterministic, template-based message built from stored prescription data (not freely generated by the LLM), naturally localized EN/Hindi-Hinglish, after which Arjun continues the conversation naturally.

Decisions resolved 2026-07-12 (v1.4), following further review of the Mind Journal and the legacy scoring surface: the old Mental Fitness scoring system (`/100` score, dimension scores, rating bands, evaluative labels, score-based recommendations, score-based prompt context, and any score-derived progress display, XP, badge, reward, ranking, or unlock) is **permanently retired for the pilot, not merely hidden** — the backend must stop creating or updating these scores entirely; the silent backend XP accrual permitted in v1.2 does **not** extend to Mental Fitness score calculation, which must stop outright · the score-free Mind Journal (§4.12) is the only pilot-visible check-in experience, and no Mind Journal entry may create or update the retired score · Arjun may receive only the **latest five** Mind Journal entries as clearly disclosed optional conversation context (resolving the earlier "latest few" ambiguity), governed by the same non-diagnostic guardrails as before · legacy scored Mental Fitness data will be permanently removed through a separate, carefully controlled cleanup — audited, dependency-checked, backed up, and count-verified before and after — that preserves all unrelated check-ins, reflections, debriefs, Mental Reps, prescriptions, safety events, cues, Focus Cards, and Playbook content, and never blindly deletes generic `CheckIn` records (§4.13); this documentation update does not perform that deletion · the existing separate Arjun founder dashboard is confirmed as the surface that will host the safety-event review workflow (§6 G5) — its exact implementation remains an engineering-plan detail.
...
The six stages. Every product surface exists to serve this loop. **[RESOLVED 2026-07-12 — v1.3]** The pilot is centered on this loop alone — the pre-authored Starter Plan sequence (multi-session plan) is hidden during the pilot and must not compete with it (§7, §8), and the Mind Journal (§4.12) is deliberately kept outside this loop as optional background context, never a loop stage.
...
| 6 | **Follow-up** | Next time the athlete opens Arjun after the practice window, the conversation resumes with the open prescription via a **deterministic template opener** (not freely generated by the LLM) — e.g. "Last time, you planned to try [practice] in [situation]. How did it go?", naturally localized EN/Hindi-Hinglish — after which Arjun continues the conversation naturally. **[RESOLVED 2026-07-12 — v1.3]** | Main chat (next-open) |
...
**Entry:** Athlete opens chat while a prescription is open (next-open rule — **no push notification, no manufactured reminder**). **[RESOLVED 2026-07-12 — v1.3]** The opening follow-up message is a **deterministic template** built from the stored prescription (practice + situation) — e.g. "Last time, you planned to try [practice] in [situation]. How did it go?", naturally localized EN/Hindi-Hinglish — never freely generated by the LLM. **Steps:** Arjun opens with this template → athlete reports → Arjun continues coaching naturally on the result: worked (reinforce, extend), partially (adjust the same practice), didn't (revisit barrier hypothesis, may re-prescribe once) → prescription closed with outcome; lesson offered to Playbook.
**Exit:** Prescription closed (worked / partial / didn't / abandoned). **Data:** prescription outcome, optional Playbook lesson.
**AC:** with an open prescription, the follow-up is Arjun's *first* conversational move on next open · the opener is deterministic and testable, not freely LLM-generated · it does not repeat on every app/chat re-open once already shown for that open prescription · outcome recorded before any new problem/prescription cycle begins · a "didn't try it yet" response keeps the prescription open without shame language ("progress comes from returning" tone, never guilt).
...
### 4.9 Mental Playbook
**Entry:** From dashboard/nav. **[RESOLVED 2026-07-12]** With the Progress tab hidden for the pilot (§7), the Playbook is the athlete's **only** pilot-facing place to review progress: cues and Focus Cards, completed practices, prescription outcomes, and reflection lessons. **[RESOLVED 2026-07-12 — v1.3]** The bottom-navigation slot vacated by the hidden Progress tab is replaced by Mental Playbook — the pilot keeps this as a navigable tab, not a reduced four-tab bar. **Steps:** Read-only review of *their* accumulation. **Portable by design** — valuable even if they stop using Arjun.
...
### 4.12 Mind Journal (private check-in) **[RESOLVED 2026-07-12 — v1.3]**
**Entry:** Optional, athlete-initiated, from dashboard/nav — never mandatory before chat, Mental Rep, Ritual, debrief, or any other feature. **Steps:** Athlete selects one or two current mental states from a short list; adds an optional short private note; entry is saved. A simple recent-entry history may be shown as a plain list (no charts, no complex analytics). **Exit:** Entry saved privately; athlete returns to whatever they were doing. **Data:** selected state(s), optional note, timestamp — private to the athlete.
**AC:** no `/100` score, no dimension score, no person rating anywhere in the experience · no evaluative labels (weak, elite, strong, building, poor, needs improvement, or equivalents) · no XP, streaks, badges, levels, milestone animations, or rewards attached to the check-in · does not gate or unlock chat, Mental Rep, Ritual, debrief, or any other feature · is never used as a pilot-success metric · remains visually and functionally separate from the diagnose → prescribe → follow-up coaching loop (§2) — it is background context, not a loop stage · the old numeric mental-fitness scoring experience (percentage score, dimension scores, rating bands, evaluative labels) is **[RESOLVED 2026-07-12 — v1.4] permanently retired, not merely hidden** — no new score of this kind may be created, updated, or used anywhere, including as chat context, a progress display, or a pilot-success measure; legacy scored records are addressed separately (§4.13).

**Recent-reflection context in chat.** **[AMENDED 2026-08-22 — supersedes the "latest five" limit resolved 2026-07-12 (v1.4)]** Arjun may receive only the athlete's **latest 10 reflections** as optional background context — informing tone and warmth, never a substitute for the focused questions and barrier confirmation in §4.3, and never sufficient on its own to trigger a hypothesis or produce a prescription. The limit is **10 records in total**, counted across the athlete's whole reflection history rather than per source. The athlete is clearly informed, at the Mind Journal surface, that their latest 10 reflections may be used as context in conversations with Arjun. No long-term scoring, profiling, personality inference, or hidden ranking may be derived from reflections. The retrieval mechanism is an engineering-plan decision — this specification fixes only the limit, at 10 in total.

### 4.13 Legacy Mental Fitness data retirement **[RESOLVED 2026-07-12 — v1.4]**
The old Mental Fitness scoring system is permanently retired (§4.12); its legacy scored data must be permanently removed, but **only through a separate, carefully controlled implementation and data-cleanup PR** — this specification does not perform that deletion and does not design its schema, scripts, or migration commands. That future work is required to include:
- an audit of every table, field, route, prompt section, report, and UI surface that creates, reads, derives, or displays Mental Fitness scores;
- a dependency check confirming no live code path still depends on the data before any deletion;
- a backup or export of the affected legacy scored records before deletion;
- deletion-count reporting, before and after cleanup, for verification;
- deletion of legacy `MentalFitnessEntry` scored records;
- deletion of any other record type **only** when it is clearly identifiable as generated solely by the retired scoring system — **no blind deletion of all generic `CheckIn` records**;
- preservation of unrelated check-ins, reflections, debriefs, Mental Reps, prescriptions, safety events, cues, Focus Cards, and Playbook content;
- verification, after cleanup, that no live code still depends on the deleted data and that new Mind Journal entries (§4.12) are unaffected.

**AC:** the cleanup ships as its own reviewed PR or controlled migration step, never bundled into an unrelated change · a backup exists before deletion · deletion counts are reported before and after · no unrelated athlete data (per the preservation list above) is touched · post-cleanup verification confirms the app functions unaffected and new Mind Journal entries are untouched.
...
**5.3 Overdiagnosis guards.** Hypothesis language only ("sounds like", "I think what might be happening") · one barrier at a time · athlete confirmation required before prescription · Arjun may say "I'm not sure yet — tell me about the next match and we'll look again" · barriers are behaviour patterns, never identity statements about the athlete · **[AMENDED 2026-08-22 — was "latest five", resolved 2026-07-12 (v1.3)]** the latest 10 Mind Journal reflections (§4.12) may inform tone as background context only — they never determine a barrier hypothesis and never trigger a prescription on their own; the barrier must always come from this conversation's focused questions and the athlete's explicit confirmation.
...
6. Paused features are invisible to pilot athletes: games, skill paths, streaks/XP/badges/levels/scores/ratings/milestones/reward animations, weekly reports, visualization entry points, daily-rep dashboard obligation, Quick Chat, and **[RESOLVED 2026-07-12]** the entire Progress tab (hidden — no pilot-visible navigation or entry point; old route/code may remain dormant, undeleted; **[RESOLVED 2026-07-12 — v1.3]** its bottom-navigation slot is replaced by Mental Playbook). **[RESOLVED 2026-07-12 — v1.3]** The Starter Plan card and its pre-authored multi-session sequence are also hidden from the Dashboard — the underlying Plan/PlanSession code and data may remain dormant, but must not be deleted or expanded, and must not compete with the core diagnose → prescribe → follow-up loop. **[RESOLVED 2026-07-12 — v1.4]** The old numeric mental-fitness scoring experience (`/100` score, dimension scores, rating bands, evaluative labels) is **permanently retired, not merely hidden** — the backend must stop creating or updating these scores entirely, and legacy scored records are removed only through the separate controlled cleanup at §4.13; the Mind Journal itself remains, as a separate score-free private check-in (§4.12). Backend XP accrual may continue silently (to minimize backend risk) but must never be visible, must never gate or unlock anything, must never influence coaching recommendations, and must never be used as a pilot-success metric — **[RESOLVED 2026-07-12 — v1.4]** this XP allowance does not extend to Mental Fitness score calculation, which must stop outright.
...
| Streaks / XP surfaces (visible) | Healthy Hook ethics: clarity is the reward, not numbers. **[RESOLVED 2026-07-12]** Backend XP accrual may continue silently to minimize backend risk, but no XP, streak, badge, level, score, rating, milestone, or reward animation may be visible, gate any feature, affect coaching, or serve as a pilot-success metric; no new gamification functionality may be built. **[RESOLVED 2026-07-12 — v1.4]** This XP allowance does not extend to Mental Fitness score calculation, which is permanently retired (see the Old numeric mental-fitness score row below) |
| Progress tab (pilot) | **[RESOLVED 2026-07-12]** Hidden entirely for the pilot — the Mental Playbook (§4.9) is the sole athlete-facing progress-review surface; the existing Progress route/code may remain dormant, undeleted, but has no pilot-visible navigation or entry point. **[RESOLVED 2026-07-12 — v1.3]** Its bottom-navigation slot is replaced by Mental Playbook |
| Starter Plan (pilot) | **[RESOLVED 2026-07-12 — v1.3]** Hidden from the Dashboard during the pilot — the pre-authored multi-session sequence must not compete with the core coaching loop; Plan/PlanSession code and data may remain dormant, not deleted or expanded |
| Old numeric mental-fitness score | **[RESOLVED 2026-07-12 — v1.4]** Permanently retired, not merely hidden — the `/100` score, dimension scores, rating bands, and evaluative labels must no longer be created or updated anywhere in the backend; legacy scored records are removed only via the separate controlled cleanup (§4.13); the Mind Journal itself remains as a separate, score-free private check-in (§4.12) outside the required coaching loop |
...
**Resolved 2026-07-12 (v1.3), completing the implementation-planning decision set:** the bottom-navigation slot vacated by the hidden Progress tab is replaced with Mental Playbook — the pilot keeps a navigable tab in that slot, not a reduced four-tab bar · the Starter Plan card and its pre-authored multi-session sequence are hidden from the pilot Dashboard, kept dormant (not deleted or expanded), and must not compete with the core diagnose → prescribe → follow-up loop · the Mind Journal remains in the pilot, rebuilt as a separate, score-free private check-in (one/two selected states + optional note, saved privately, optional plain-list history) — no `/100` score, dimension score, person rating, or evaluative label, no XP/streak/badge/level/milestone/reward, never mandatory, never gating, never a pilot-success metric, and kept clearly separate from the required coaching loop; Arjun may receive only the latest few entries as limited, disclosed background context that informs tone only and never determines a barrier or produces a prescription on its own · the next-open follow-up opener is a deterministic, template-based message built from stored prescription data (practice + situation), naturally localized EN/Hindi-Hinglish, never freely generated by the LLM, after which Arjun continues the conversation naturally.

**Resolved 2026-07-12 (v1.4):** the old Mental Fitness scoring system — `/100` score, dimension/category scores, rating bands, evaluative labels, score-based recommendations, score-based prompt context, score-derived progress displays, and any score-derived XP/badges/rewards/rankings/unlocks — is **permanently retired for the pilot, not merely hidden**; the backend must stop creating or updating these scores entirely, and the v1.2 allowance for silent backend XP accrual does **not** extend to score calculation · the score-free Mind Journal is the only pilot-visible check-in experience, and no new entry may create or update the retired score · Arjun may receive only the **latest five** Mind Journal entries as clearly disclosed optional context, resolving the earlier "latest few" ambiguity, under the same non-diagnostic guardrails as before · legacy scored Mental Fitness data will be permanently removed only through a separate, controlled implementation and data-cleanup PR — audited, dependency-checked, backed up, and count-verified, preserving all unrelated check-ins, reflections, debriefs, Mental Reps, prescriptions, safety events, cues, Focus Cards, and Playbook content, with no blind deletion of generic `CheckIn` records; this documentation update performs no deletion · the existing separate Arjun founder dashboard is confirmed as the reused surface for the safety-event review workflow, supporting pending events, category and risk level, athlete identifier, guardian contact, source context, structured review actions, and review timestamps, with no permanently stored AI incident narrative — its exact implementation remains an engineering-plan detail.
...
**End of specification — v1.4 (Approved — Frozen MVP).**
