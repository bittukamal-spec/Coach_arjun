  ...
  try {
    ...
    if (process.env.ANTHROPIC_API_KEY && await isTrialActive(req.userId)) {
      ...
      const msg = await anthropic.messages.create({
        ...
        messages: [{
          role: 'user',
          content: `You are Arjun, a direct and warm sports coach for a young Indian ${sport} athlete. Write a 3–4 sentence personal report based on their mental fitness scores today. Structure it as: (1) acknowledge what the numbers show — name the one strongest and one weakest dimension specifically, (2) explain briefly what that pattern means for training or competition today, (3) end with a single sentence recommending ONE of these in-app tools based on their weakest area: "Try Pressure Reset" (low calm), "Try Focus Lock" (low focus), "Build a Focus Card" (low self-talk or confidence), "Try Reset Rally" (low bounce-back), "Do a Training Review" (low drive), or "Talk to me" (low mood). Be specific to the numbers, not generic. No bullet points. No praise padding. Address them directly as "you".${langNote}
