...
// Recovery is triggered by a staged coaching TRANSITION now — the reply-chip
// tool that used to trigger it no longer exists.
function proposeToolResponse(draftText, input) {
  return { stop_reason: 'tool_use', content: [
    ...(draftText ? [{ type: 'text', text: draftText }] : []),
    { type: 'tool_use', id: 'tu-p', name: 'propose_barrier', input },
  ] };
}
const RECOVERY_PROPOSE_INPUT = {
  problemStatement: 'Thinking about the result mid-innings',
  barrierHypothesis: 'Attention moves to the scoreboard under pressure',
};
...
test('a second empty recovery response still produces the EMPTY_FINAL_TEXT deterministic-retry condition — sanitizeFinalText(loop.finalText) is null, exactly as chat.js checks', async () => {
  const anthropic = makeAnthropicStub([
    proposeToolResponse('draft', RECOVERY_PROPOSE_INPUT),
